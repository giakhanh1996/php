...
<?php
    ...
    $stree="select so_luong from kho_tt where ma_hang='$mat_hang'";
    $sqlee=mysql_query($stree);
    $rowee=mysql_fetch_assoc($sqlee);
    $s_ll=$rowee['so_luong'];
    $sl_kd=2;
    if(isset($mat_hang) && isset($so_luong) && $so_luong != "" && $mat_hang != "")
    {
        if($so_luong <= $s_ll)
        {
            $str3="insert into kho_ao2(ma_vat_tu,so_luong) values('$mat_hang','$so_luong')";
            ...
            {
                $str5="update kho_ao2 set so_luong = so_luong+'$so_luong' where ma_vat_tu='$mat_hang'";
                mysql_query($str5);
            }
            $sl_kd=0;
        }
    ...
    if(isset($chon))
    {
        ...
        {
            $id_del=$chon[$i];
            $str6="delete from kho_ao2 where ma_vat_tu='$id_del'";
            mysql_query($str6);
        }
    }
    ...
    <head>
        ...
        <script type="text/javascript">
            function hienthihang(){
                ...
                window.location="themhangvaohoadon.php?ma_loai="+get;
                //alert(get);
            }
        </script>
    ...
    <body style="margin:0;padding:0;">
        <form action="<?php echo $SCRIPT_NAME; ?>" method="post" style="margin:0;padding:0;" name="list_hang">
            <div style="height:205px;border:1px solid #000000;overflow:auto;">
                    ...
                    <table width="100%" border=0 cellpadding=0 cellspacing=0>
                        ...
                        <?php
                            $str2="select * from kho_ao2 order by ma_vat_tu";
                            ...
                            while($row2=mysql_fetch_assoc($sql2))
                            {
                                ...
                                $tht=$row4['don_gia']*$row2['so_luong'];
                                echo "<tr><td style='width:30px;height:20px;border-right:1px solid #000000;text-align:center;' class='border-td'><input type='checkbox' name='chon[]' value='".$row2['ma_vat_tu']."'></td><td class='border-td-2' style='text-align:center;border-right:1px solid #acabab;width:65px;'>".$row2['ma_vat_tu']."</td><td class='border-td-2' style='border-right:1px solid #acabab;width:160px;'>".$row4['ten_hang']."</td><td class='border-td-2' style='border-right:1px solid #acabab;width:60px;'>".$row4['don_vi_tinh']."</td><td class='border-td-2' style='border-right:1px solid #acabab;width:150px;'>".$row4['don_gia']."VND</td><td class='border-td-2' style='text-align:right;color:red;font-weight:bold;width:100px;border-right:1px solid #acabab;'>".$row2['so_luong']."</td><td class='border-td-2' style='text-align:right;color:red;font-weight:bold;'>".$tht." VND</td></tr>";
                            }
                        ?>
                        <tr><td style="width:30px;height:20px;border-right:1px solid #000000;" class="border-td">&nbsp;</td><td class="border-td-2" style="border-right:1px solid #acabab;">&nbsp;</td><td class="border-td-2" style="border-right:1px solid #acabab;">&nbsp;</td><td class="border-td-2" style="border-right:1px solid #acabab;">&nbsp;</td><td style="border-right:1px solid #acabab;" class="border-td-2">&nbsp;</td><td class="border-td-2" style="border-right:1px solid #acabab;">&nbsp;</td><td class="border-td-2">&nbsp;</td></tr>
        ...
        <div>
            ...
            <?php
                if($sl_kd==1)
                {
                    echo"<script type='text/javascript'>alert('So luong khong du !!');</script>";
                }
            ?>
        </div>
    </body>
